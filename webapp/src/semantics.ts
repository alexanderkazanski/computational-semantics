...
const TEMPORAL_CUES: Record<string, string> = {
  yesterday: "past",
  today: "present",
  tomorrow: "future",
  now: "present",
...
const isContentWord = (lemma: string): boolean =>
  /^[a-z]+$/.test(lemma) && !DETERMINERS.has(lemma) && !PRONOUNS.has(lemma);

const guessPos = (lemma: string): string => {
...
const pickMainVerb = (tokens: Token[]): Token | null =>
  tokens.find((token) => guessPos(token.lemma) === "verb") ?? null;

const guessSubject = (tokens: Token[], verbIndex: number | null): string | null => {
...
const guessObject = (tokens: Token[], verbIndex: number | null): string | null => {
  if (verbIndex === null) return null;
  const candidates = tokens
    .slice(verbIndex + 1)
    .filter((token) => ["noun", "pronoun"].includes(guessPos(token.lemma)));
...
const sentimentScore = (tokens: Token[]): number =>
...
const collectTopics = (tokens: Token[]): string[] => {
  const topics: string[] = [];
...
export const analyzeSentence = (sentence: string): Analysis => {
